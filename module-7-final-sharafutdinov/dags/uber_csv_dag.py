...
def load_and_clean_csv(**context):
    """Загрузка CSV из S3 - упрощенная версия"""
    ...
    df = None
    encodings = ['utf-8-sig', 'utf-16', 'latin1', 'cp1251']
    ...
    for encoding in encodings:
        try:
            ...
            print(f"Успешно прочитано в кодировке: {encoding}")
            break
    ...
    for col in df.columns:
        if 'Unnamed' in str(col) or str(col).startswith('2015') or str(col).startswith('2009') or ':' in str(col):
    ...
    # Проверяем структуру
    required_columns = ['pickup_datetime', 'fare_amount', 'pickup_longitude', 
...
with DAG(
    ...
    description='DAG для обработки данных uber',
    schedule='@hourly',
    catchup=False,
